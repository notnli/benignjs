...
$(function() {
    $.extend({
        brand: function(main, sub, btn, json) {
            var data = json['data'], tempHTML = [ "<li>ŃˇĆ·ĹĆ</li>" ], temp_id = [], temp_name = [];
            for (var obj in data) {
            ...
            $(main).on("click", "li:not(.sgroup)", function(e) {
                ...
                $(sub).empty().removeAttr("disabled").append("<li>ŃˇłµĐÍ</li>");
                ...
                function afterGetData(json){
                    ...
                    for(var obj in data){
                        ...
                        if(temp_id.length == 0){
                            tempHTML.push('<li value="">ÔÝÎŢĎŕąŘłµĎµ</li>');
                        }else{
            ...
            if (btn && btn != "") $(btn).click(function() {
                ...
                if (bid != "ŃˇĆ·ĹĆ" && bid != "" && bid !== "0" && !!bid) {
                    if (subid != "ŃˇłµĐÍ" && subid != "" && subid !== "0" && !!subid) {
