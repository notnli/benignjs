...
function validateFormAndCalculatePersonal() {
    ...
    const interest_rate = rangePersonal[document.getElementById("interest-rate-personal").value];
    ...
    const err_interest_rate = validateRangeInput(interest_rate_input, rangePersonal[rangePersonal.length - 1], rangePersonal[0], 'error-interest-rate-personal');
    ...
    if (err_loan_amt, err_tenure_months, err_interest_rate) {
        const roi_per_month = ((interest_rate) / (12 * 100));
        const roi_1 = (1 + roi_per_month)
        const emi_amt = (loan_amt.value * roi_per_month * (Math.pow(roi_1, tenure_months) / (Math.pow(roi_1, tenure_months) - 1)));
        const interest_payable = (emi_amt * tenure_months) - loan_amt.value;
