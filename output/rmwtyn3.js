...
RMWSSO.getUserData(function(user){
	if(!user.userId){
		//$("#txz_dlq").show();
		//$("#txz_dlh").hide();
		//console.log('śú™ÁôĽŚĹē');
	}else{
		//$("#txz_dlq").hide();
		$("#txz_dlh").show();
		var loginMsg = "<a href='http://sso.people.com.cn/login' target='_blank'>"+user.nickname+"</a><br />ś¨ĘŤŅéśā®";
		$("#loginMsg").html(loginMsg);
		//alert('ŚĹďŚČćÁôĽŚĹēÁĒ®śą∑Ôľö' + user.nickname);
		$("#login_button").html("<em></em>"+user.nickname.substring(0,2)+"‚Ä¶");
	}
});
...
$("#logout").bind('click', function(event){
	RMWSSO.logout(function(){
		//alert('ťÄÄŚáļśąźŚäüÔľĀ')
		//$("#txz_dlq").show();
		//$("#txz_dlh").hide();
		$("#p_login").css({"display":"none"});
		$(".rm_nav").css("height","55px");
		$("#login_button").html("<em></em>"+"<a href=\"http://sso.people.com.cn/login\" target=\"_blank\">ÁôĽŚĹē</a>");
	});
})
$(document).ready(function() {
	...
	$(window).on("load", function () {		
		$("#login_button").bind("mouseenter", function() {
			if($("#login_button").text().indexOf("ÁôĽŚĹē") == -1){
				$(".rm_nav").css({height:167});
				$("#p_login").show();
			}
