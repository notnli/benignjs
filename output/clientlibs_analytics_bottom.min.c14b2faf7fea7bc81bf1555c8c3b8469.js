...
vp.getNCCampaignId=function(){return typeof this.fcid==="string"?this.fcid.split("|")[0]:""
};
vp.getHashedEmail=function(){return this.getProperty("sha")
};
...
var c={};
c.pageLoad=function(n,o,m){};
c.event=function(){};
c.trackLink=function(o,m,n){};
window.MockTagManager=c;
function g(n){var o=[],m=1;
n.forEach(function(p){var q=p.divId;
o.push({name:"div_"+q,list:q,position:m,dimension60:p.formDriverIdList.join("|"),dimension94:q,dimension95:p.campaignUrl,metric15:1});
m++
});
return o
}function k(){var m=digitalData.component||[{}],o=m.length,t={event:"promotionImpression",ecommerce:{promoView:{promotions:[]}}},n=[];
if(o===0){return t
}var s=o,p=m[o-1].componentInfo,r=p.campaign||"",q=p.experience||"";
do{s=s-1;
p=m[s].componentInfo;
n.push({id:p.componentID||"",name:r,creative:q,position:(p.itemType||"")+"|"+(p.position)})
}while(m[s-1]&&(m[s-1].componentInfo.campaign===r)&&(m[s-1].componentInfo.experience===q));
t.ecommerce.promoView.promotions=n;
return t
}function l(){var n=b.personalization,m={event:"promotionClick",ecommerce:{promoClick:{promotions:[]}}};
if(!n){return m
}m.ecommerce.promoClick.promotions=[{id:n.clickItemId||"",name:n.clickCampId||"",creative:n.clickExpId||"",position:(n.clickItemType||"")+"|"+(n.clickItemPos||"")}];
return m
}function a(m,p){var n=b.media,o={event:m};
p.forEach(function(q){if(q in d){o[q]=n[d[q]]
}});
return o
}function f(){var m="";
try{if(embedded_svc&&embedded_svc.settings&&embedded_svc.settings.extraPrechatFormDetails){var n=embedded_svc.settings.extraPrechatFormDetails.filter(function(p){return p.label=="Driver Campaign"
});
m=n.length>0?n[0].value:""
}}catch(o){}return m
}function j(){var n="";
try{if(embedded_svc&&embedded_svc.settings&&embedded_svc.settings.extraPrechatFormDetails){var m=embedded_svc.settings.extraPrechatFormDetails.filter(function(p){return p.label=="CM_OMTRJoinID"
});
n=m.length>0?m[0].value:""
}}catch(o){}return n
}digitalData.util.mappings.google={register:function(m,o){if(typeof m==="string"){var n=m.toLowerCase();
if(!e[n]){e[n]=o
}}return this
},getEvent:function(m){if(typeof m==="string"){var n=m.toLowerCase();
var o=e[n];
...
}).register("chat_view",function(){var n=b.customlink;
var m=n==="chat-invite"?"PopUp":n==="chat-anchor"?"Anchor":"CTA";
return{event:"custEv_chatView",chatLocation:m,nameCaptureCampaignId:f(),formJoinIDUnique:j()}
}).register("chat_init",function(){var n=b.customlink;
var m=n==="chat-invite"?"PopUp":n==="chat-anchor"?"Anchor":"CTA";
return{event:"custEv_chatInitialization",chatLocation:m,nameCaptureCampaignId:f(),formJoinIDUnique:j()}
...
}).register("promo_impression",function(){return k()
}).register("promo_click",function(){return l()
...
}var C=digitalData.util,i=digitalData.page,c=digitalData.user[0].profile[0],z="form[data-sfdc-form]",j="data-form-environment-type",q="https://hosted-scratch.herokuapp.com/trial",b={modal:"Modal",dropPanel:"Drop panel",swapContainer:"Swap container",onPage:"On page"},h="sfdcForm",u=function(D){return D[h].formType==="lead"
},d=function(D){return !!(D[h].signup)
},k=function(D){return !!(D&&D.action.indexOf(q)>-1)
},m=function(D){return D.classList.contains("dynamic-load")
},f=function(E){var D="invalidFields";
return D in E&&!E[D].length
},B=function(){var D=JSON.parse(sessionStorage.getItem("form_session"));
return !!(D&&D.twoStep)
},A=function(G){var F=C.getFormFieldValue(G,"CompanyEmployees"),H=C.getFormFieldValue(G,"CompanyCountry"),D=C.getFormFieldValue(G,"CompanyName"),E=C.getFormFieldValue(G,"Lead.Primary_Product_Interest__c");
c.usercompanysize=F?F:"[FORM:NO COMPANY SIZE]";
c.usercompanycountry=H?H:"[FORM:NO COUNTRY]";
c.usercompanyname=D?D:"[FORM:NO COMPANY NAME]";
c.userprimaryprodinterest=E?E:"[FORM:NO PRIMARY PRODUCT INTEREST]";
c.userfirstformcompletetime=vp.getFirstFormCompleteTime()
},n=function(F){var H="FormCampaignId",D=F.elements[H],G=F.elements.UserEmail,E=Util.getParam(H)||(D?D.value:"");
if(E&&typeof(SHA1)!=="undefined"&&G){vp.fcid=Util.convert15To18(E)+"|"+SHA1(G.value)
}return vp.fcid
},v=function(F){var G="FormCampaignId",D=F.elements[G],E=Util.getParam(G)||(D?D.value:"");
if(E){c.nameCaptureCampaignId=Util.convert15To18(E)
}},g=function(D){var E=D.elements.UserEmail;
if(typeof(SHA256)==="function"&&E&&E.value){c.hashedEmailAddress=SHA256(E.value)
}},l=function(E){var D=(C.getFormFieldValue(E,"subname")?C.getFormFieldValue(E,"subname"):false);
return(D&&D==="RBNBlog")
},x=function(D){Page.leadCaptureForms.push(D);
C.addPageData("type",Page.getType())
},y=function(){i.convertingpage=i.pagename;
i.convertingpageurl=i.url
},t=function(D){var L=[],F=d(D),K=k(D),G=l(D),I=F||K?"syncSignupFormCompleted":"Form Completion";
c.transactionid=n(D);
v(D);
g(D);
A(D);
if(G){C.addPageData("is_roll_based_newsletter","true");
var H=[],M=document.querySelectorAll(".field input[type=checkbox]:checked");
if(M){for(var J=0;
J<M.length;
J++){H.push(M[J].name)
}var E=H.join(", ");
C.addPageData("newsletter_subscriptions",E)
}}if(c.transactionid){if(C.readCookie("c30")){i.offerid=C.readCookie("c30")
}else{i.offerid=i.offerid?i.offerid:"[NO OFFER ID]"
}i.numberofformcompletes="+1";
L.push("formcompletesubmitbuttonclick");
if(F||K){L.push("signupformcomplete")
}if(K){L.push("formbutton_clicks")
}}C.deleteCookie("v0");
C.deleteCookie("v20");
C.deleteCookie("convertingPageUrl");
C.deleteCookie("v22");
C.deleteCookie("v28");
C.deleteCookie("c30");
C.trackBehavior(I,L,C.formDataPoints.concat(["user[0].profile[0].usercompanysize","user[0].profile[0].usercompanycountry","user[0].profile[0].userprimaryprodinterest","user[0].profile[0].usercompanyname","user[0].profile[0].userfirstformcompletetime","user[0].profile[0].transactionid","page.numberofformcompletes"]))
},w=function(){C.trackBehavior("Form Button Click",["formbutton_clicks"],["user[0].profile[0].userfirstformcompletetime"])
},a=function(D,J,F){var I=Page.getLocale(),K="",G="",H=[];
C.addPageData("formerrors","");
C.addPageData("formerrorsall","");
C.addPageData("formerrorspercentage","");
if(!D){return false
}D.forEach(function(M){var L=(I+":"+M);
if((K+L).length<100){K+=(K===""?"":"|")+L
}if((G+L).length<255){G+=(G===""?"":"|")+L
}});
if(K){K=K.toLowerCase();
G=G.toLowerCase();
var E=Math.round(J/(F+J)*100).toString()+"%";
C.addPageData("formerrors",K);
H.push("page.formerrors");
C.addPageData("formerrorsall",G);
H.push("page.formerrorsall");
C.addPageData("formerrorspercentage",E);
H.push("page.formerrorspercentage");
C.trackBehavior("Form error tracking",["form_errors"],H)
}},s=function(D){if(i.currentFormStep&&D>i.currentFormStep&&i.totalFormStep){i.currentFormStep=D;
C.addBehavior("stepbystepformprogress")
}},e=function(E,F){var D=E.querySelector("[type=submit]").innerText.trim();
C.addPageData("twostepbuttontext",D);
C.addPageData("twostepfieldname",F);
C.addBehavior("two_step_form_submit")
},p=function(K,D){var I="Step by step",F="Stacked",H="Two Step",G="onPage",J=K.sfdcForm.paginated?I:F,E=K.classList.contains(r.dynamicLoad)?K.getAttribute(j):G;
if(typeof(D)==="boolean"&&D){J=H
}C.addPageData("form_experience_type",J);
C.addPageData("form_environment_type",b[E]);
if(J===I&&K[h].paginate){i.currentFormStep=-1;
i.totalFormStep=K[h].paginate.total;
s(1);
K.addEventListener(r.stepProgress,function(M){var L=parseInt(M.detail);
if(L){s(L)
}})
}},o=function(){try{digitalData.page.formJoinIDUnique=Util.createUUID()
}catch(D){}},r={init:"init-form",submit:"submit-form",invalid:"invalid-form",error:"error-form",success:"success-form",failed:"failed-form",closeParent:"close-parent",dynamicLoad:"dynamic-load",stepProgress:"step-progress",twoStepProgress:"two-step-progress"};
C.formDataPoints=["page.url","page.referrer","page.locale","user[0].profile[0].dayssincelastvisit","page.pagename","user[0].profile[0].targetingcompanysize","user[0].profile[0].usertype","page.convertingpage","page.internaldrivers","user[0].profile[0].timepartinghour","user[0].profile[0].timepartingday","user[0].profile[0].formfillstatus","page.scampaign","page.offerid","user[0].profile[0].visitnumber","page.cloud","page.type","page.prevpage","user[0].profile[0].targetingcompanyindustry","page.formname","page.sembrand","user[0].profile[0].targetingcompanyname","user[0].profile[0].targetingaudience","user[0].profile[0].accounttype","user[0].profile[0].kxsfdc","user[0].profile[0].hassmbinference","page.form_environment_type","page.form_experience_type","page.partnerpromocode","page.drivertypepathing","user[0].profile[0].detailedusertype","page.prevpagesanitized","page.namenolocale","page.drivertype","page.uiframework","page.segment","page.reportsuite","page.convertingpageurl"];
C.components.trackFormV2=function(){var D=this;
$(document).on(r.dynamicLoad,z,function(E){this.classList.add(r.dynamicLoad);
this.setAttribute(j,E.detail)
});
$(document).on(r.init,z,function(J){var I=this;
if(!I[h]){return
}o();
v(I);
var E=k(I),F=m(I),H=u(I)||E;
if(H){x(I);
D.trackConverting();
if(F){y()
}p(I,B());
if(window.IntersectionObserver!==null){var G=new IntersectionObserver(function(L){if(L[0].isIntersecting===true){D.trackFormView(I)
}},{threshold:[0]});
G.observe(I);
digitalData.page.observer=G
}else{D.trackFormView(I)
}I.addEventListener(r.success,function(){if(F){y()
}p(I);
t(I)
});
I.addEventListener(r.submit,function(){if(F){y()
}if(E&&f(I)){t(I);
return
}w()
});
I.addEventListener(r.error,function(N){if(F){y()
}var O=N.detail;
if(!O){return
}var M=O.fields,L=O.numValid;
if(M&&typeof L==="number"){a(M,M.length,L)
}})
}else{if(I[h].twoStep){I.addEventListener(r.twoStepProgress,function(L){var M=L.detail;
e(I,M)
})
}else{var K=I[h].submitEvent;
if(K){I.addEventListener(r.submit,function(){C.addPageData("formCustomEv",K);
C.trackBehavior("form_nonlead_submission",["form_nonlead_submission"],["page.formCustomEv"])
})
}}}})
}
}());
...
(function(){if(typeof digitalData==="undefined"||typeof Page==="undefined"||typeof vp==="undefined"||typeof Util==="undefined"){return
}var h=digitalData.util,a=digitalData.page,g=digitalData.user[0].profile[0],b=function(){if(vp.getNCCampaignId()){g.nameCaptureCampaignId=vp.getNCCampaignId()
}},d=function(){if(vp.getHashedEmail()){g.hashedEmailAddress=vp.getHashedEmail()
}},f=function(){try{var k=Util.getParam("sfuuid");
a.formJoinIDUnique=k||a.formJoinIDUnique||Util.createUUID()
}catch(l){}},j=function(){let navType=window.performance.getEntriesByType("navigation")[0].type;
if("navigate"!==navType){return
}var k=Util.convert15To18(Util.getParam("d")?Util.getParam("d"):Util.getParam("DriverCampaignId"));
f();
if(k&&!Util.getParam("d")){var l=h.getReferrerPageName()+"|"+k;
h.addPageData("persistedInternalDriver",l);
h.addProduct(k,"intdriverclick");
h.addPageData("internaldrivers",l);
h.addBehavior("intdriverclick")
}a.offerid=Util.convert15To18(Util.getParam("nc")?Util.getParam("nc"):Util.getParam("FormCampaignId"));
if(a.offerid){a.offerid="form-builder|"+Page.calculateName(Util.getParam("landing_page"))+"|"+a.offerid
}if(vp.getFormCompleteId()){g.transactionid=vp.getFormCompleteId()
}b();
d();
if(g.transactionid){a.numberofformcompletes="+1";
h.addBehavior("autoformcomplete")
}},i=function(){let navType=window.performance.getEntriesByType("navigation")[0].type;
if("navigate"!==navType){return
}var k=(h.readCookie("appxud")?h.readCookie("appxud"):[]);
var l=Util.convert15To18(k.d?k.d:"");
f();
if(l){var m=h.getReferrerPageName()+"|"+l;
h.addPageData("persistedInternalDriver",m);
h.addProduct(l,"intdriverclick");
h.addPageData("internaldrivers",m);
h.addBehavior("intdriverclick")
}a.offerid=Util.convert15To18(vp.getFormCompleteId());
if(a.offerid){a.offerid="form-builder|"+Page.calculateName(Util.getParam("landing_page"))+"|"+a.offerid
}if(vp.getFormCompleteId()){g.transactionid=vp.getFormCompleteId()
}b();
d();
if(g.transactionid){a.numberofformcompletes="+1";
h.addBehavior("autoformcomplete")
}},c=function(){h.setFormFillVals()
},e=function(){if(h.hasEvent("formcompletesubmitbuttonclick")){if(!h.readCookie("v22")){a.internaldrivers="[NO INTERNAL PLACEMENT ID]"
}h.deleteCookie("v0");
h.deleteCookie("v20");
h.deleteCookie("convertingPageUrl");
h.deleteCookie("v22");
h.deleteCookie("v28");
h.deleteCookie("c30")
}};
h.components.trackFormConfPage=function(){if(Page.isSkipFormRedirect()){c();
this.trackConverting();
j();
e()
}document.addEventListener("inline-form-skip-success",function(k){c();
h.components.trackConverting();
i();
e()
})
}
...
(function(){if(!window.optimizelyEdge||!SfdcWwwBase.gdpr.isAdvertisingActive()){return
