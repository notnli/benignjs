...
function setCookie(name, value,expires)   
{
    if(expires!=null)   
   {
       ...
       LargeExpDate = calcTime(expires, 24*365); 
   }   
   document.cookie = name + "=" + escape (value)+((expires == null) ? "" : ("; expires=" +LargeExpDate.toGMTString())) + ";path=/;domain=.caijing.com.cn";
}
/* 计算cookie的过期时间 */
function calcTime(expireDays, offset) {
	// create Date object for current location
	d = new Date();
	d.setTime(d.getTime()+(expireDays*3600*1000*24));
	utc = d.getTime() + (d.getTimezoneOffset() * 60000);
	nd = new Date(utc + (3600000*offset));
	return nd;
}

function onlyDeleteCookie(){
...
//cookie操作
function deleteCookie(backUrl){
	var name = "SA_USER_NICK_NAME";
	var mainUrl = "http://service.caijing.com.cn";
	var logoutHref1 = mainUrl + "/logout/index/nurl/"+backUrl;
	onlyDeleteCookie();
	window.location.href=logoutHref1;
}
...
function loginRegister(url){
					...
					if(url==null || url=='undefined'){
						backUrl = 'YUhSMGNEb3ZMM2QzZHk1allXbHFhVzVuTG1OdmJTNWpiaTlwYm1SbGVDNW9kRzFz';
					}else{
						backUrl = url ;
					}
					
					var cookieStr = getCookie('SA_USER_NICK_NAME');
					if (cookieStr){document.write("<a class='quit' style='float:right;color:#023F80' href='javascript:deleteCookie(backurl);' target='_self'>退出</a> <span style='color:#25a5ff;float:right'> 　"+cookieStr+"&nbsp;</span>");}
					else{document.write("<ul class='login'><li class='login_in'><a href='http://service.caijing.com.cn/usermanage/login/url/"+backUrl+"' target='_self' rel=\"nofollow\">登录</a></li> <li><a href='http://service.caijing.com.cn/auth/register' target='_self'>注册</a></li></ul>");}
}
...
/*焦点图*/
var Tabs=function(oEle,btn,list,aBtnStyle,aListStyle){
	
	var _this=this;
	this.oTimer=null;
	this.aBtnStyle=aBtnStyle;
	this.aListStyle=aListStyle;
	this.currentIndex=0;
	this.oEle=oEle;
	this.btn=this.getClass(this.oEle,list);
	this.list=this.getClass(this.oEle,btn);
	this.aA=this.btn.getElementsByTagName("span");
	this.aBtn=this.btn.getElementsByTagName("a");
	this.aDiv=this.list.getElementsByTagName("div");
	this.maxLength=this.aDiv.length;

	for(var i=0;i<this.aBtn.length;i++){
		this.aBtn[i].index=i;
		
		this.aBtn[i].onmouseover=function(){
			_this.doChange(this.index);
			clearInterval(_this.oTimer);
		}
		this.aBtn[i].onmouseout=function(){
			_this.oTimer=setInterval(
				function(){
					_this.doMove();
				}
			,8000);
	...
	if(this.aA.length >0){
		this.aA[1].onclick=function(){
			...
			if(_this.currentIndex<0){
				_this.currentIndex=(_this.maxLength-1);
			}
			_this.doChange(_this.currentIndex);			
			return false;
		};
	}
	if(this.aA.length >0){
		this.aA[0].onclick=function(){
			_this.currentIndex++;
			if(_this.currentIndex>(_this.maxLength-1)){
...
Tabs.prototype.getClass=function(obj,tags){
	var oTags=obj.getElementsByTagName("*");
	for(var i=0;i<oTags.length;i++){
		if(oTags[i].className==tags){
			return oTags[i];
		}
	}
};

Tabs.prototype.startMove=function(){
	var _this=this;
	if(this.oTimer){
...
Tabs.prototype.doMove=function(){
	this.currentIndex++;
	var index=this.currentIndex % this.maxLength;
	this.doChange(index);
};

Tabs.prototype.doChange=function(index){
	for(var j=0;j<this.aBtn.length;j++){
		this.aBtn[j].className="";
		this.aDiv[j].className="";
	}
	this.aBtn[index].className=this.aBtnStyle;
	this.aDiv[index].className=this.aListStyle;
	this.currentIndex=index;
};
/*广告调用*/
function ads_display(channel,pos){
	document.writeln("<scr" + "ipt type='text/javascript' src='http://caijing.allyes.com/main/adfshow?user=caijing|"+channel+"|"+pos+"&db=caijing&border=0&local=yes&js=ie'></scr" + "ipt>");
}

/*js版焦点图*/
//定义焦点图类
function hxFoucs(panelId,picStr,mylinksStr,textsStr,moreStr,width,height){
	this.panel = hxFoucs.$(panelId);
	this.pic = picStr.split('|');
	this.mylinksStr = mylinksStr.split('|');
	this.textsStr = textsStr.split('|');
	this.len = hxFoucs.minInt([this.pic.length,this.mylinksStr.length,this.textsStr.length]);
...
hxFoucs.prototype = {
	//初始化内容
	init:function(botClass,numClass,showClass,titClass) {
		...
		hxFoucs.css(this.con,{width:this.width+'px',height:this.height+'px',overflow:'hidden',position:'relative'});
		this.contrainer.appendChild(this.con);
		//构建图片容器
		this.imgCon = document.createElement('div');
		hxFoucs.css(this.imgCon,{width:this.width+'px',height:(this.height-parseInt(res.TBHeight))+'px',overflow:'hidden'});
		var tempHTML = '';
		...
		hxFoucs.css(this.markBg,{width:this.width+'px',height:'30px',background:'#000000',opacity:'0.5',position:'absolute',bottom:(parseInt(res.TBHeight))+'px'});
		this.con.appendChild(this.markBg);
		
		//构建文字容器
		this.wordCon = document.createElement('div');
		this.wordCon.className = this.titClass;
		hxFoucs.css(this.wordCon,{position:'absolute',bottom:(parseInt(res.TBHeight))+'px',width:(this.width)+'px',height:'30px','line-height':'30px',overflow:'hidden','text-align':res.Align,'font-size':res.Size+'px','font-family':res.Font,'font-weight':res.Weight});
		...
		hxFoucs.css(this.markBg2,{width:this.width+'px',height:'20px',position:'absolute',bottom:'0px'});
		this.con.appendChild(this.markBg2);

		//构建标签层
		this.markCon = document.createElement('ul');
		this.markCon.className = this.numClass;
		hxFoucs.css(this.markCon,{position:'absolute',bottom:'0px',right:'5px',height:'20px','margin':'0px','padding':'0px'});
		this.con.appendChild(this.markCon);
		for(var j=1;j<=this.len;j++) {
			var li = document.createElement('li');
			hxFoucs.css(li,{'styleFloat':'left','cssFloat':'left','text-align':'center',overflow:'hidden','font-size':'12px','font-family':res.Font,cursor:'pointer'});
	...
	bindEvent:function() {
		...
		for(var j=0;j<imglist.length;j++) {
			hxFoucs.css(imglist[j],{position:'absolute',top:'0px',left:'0px',display:'none'});
			if(j==0) hxFoucs.css(imglist[j],{display:''});
			}
		},
	imgMouse:function(){
		var _this = this;
	...
	tab:function(i) {
		...
		hxFoucs.css(imglist[i],{opacity:'1',display:''});
		this.fade(i);
		},
	//样式回归
	cssback:function() {
	...
	fade:function(n) {
		...
		var mark = 0.04;
		hxFoucs.css(nowObj,{opacity:'1',display:''});
		hxFoucs.css(toObj,{opacity:'0',display:''});
		if(this.timer){
			clearInterval(this.timer);
			this.timer=null;
		}
		this.timer = setInterval(
			function() {
				if(mark<1) {
					hxFoucs.css(nowObj,'opacity',1-mark);
					hxFoucs.css(toObj,'opacity',mark);
					mark+=0.04;
					}
				else {
					clearInterval(_this.timer);
					hxFoucs.css(nowObj,{opacity:'0',display:'none'});
					hxFoucs.css(toObj,{opacity:'1',display:''});
					_this.showMark = n;
					//_this.auto();
					}
				},
...
hxFoucs.camelCase = 	function(str){
		return str.replace(/-\D/g, function(match){
				return match.charAt(1).toUpperCase();
			}
		);
}
hxFoucs.IE = window.ActiveXObject?true:false;
hxFoucs.css = function(ele,name,value) {
      var ele = hxFoucs.$(ele);
      if(typeof name == 'undefined' && typeof value == 'undefined') {
			return ele.style.cssText;
		}
		else if(typeof name == 'string' && typeof value == 'undefined') {
			if (name == 'opacity' && hxFoucs.IE && ele.style.filter) 
				return parseFloat(ele.style.filter.replace(/alpha\(opacity=/, '').replace(/\)/, '')) / 100;
			else {
				name = hxFoucs.camelCase(name);
