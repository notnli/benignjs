...
SfdcWwwBase.gdpr.hasAdvertising=e.Deferred();
var a={1:["liveagent_vc","liveagent_ptid","liveagent_sid","liveagent_oref","web_core_geoCountry","web_core_geoRedirected","web_core_regionSelectOverride","SSID","_dc_gtm_UA-118367394-2","_dc_gtm_UA-118367394-5","OptanonConsent","OptanonAlertBoxClosed","X-Salesforce-CHAT","videoId","guided-tour-selection","s_vi","privacy_bar","webact","AKA_A2","Lastvaliddriver"],3:["_gid","_gat","_ga","_gat_UA-118367394-2","_gat_UA-118367394-5","analytics.twitter.com","appxud","s_pers","Lp","roi","rrCookie_affiliateInfo_home","rrCookie_affiliateInfo_sales","rrCookie_affiliateInfo_service","rrCookie_affiliateInfo_pricing","rrCookie_affiliateInfo_platform","AEM Client Context","coveo_visitorId","Vidyard","Pardot","mbox","s_sess","db","l_cloud","chat","kxsfdc","ptb","ptc","version","invoca_session","cvtdt","is","iv","check","BrowserId","AMCV_8D6C67C25245AF020A490D4C%40AdobeOrg","AMCVS_8D6C67C25245AF020A490D4C%40AdobeOrg","lastvaliddriver","c22","c30","c40","v0","v4","v20","v22","v28","v32","closedInitialChat","Lastvaliddriver","Cvtdt","eventsPopupOptOut","convertingPageUrl"],4:["optimizelyEndUserId","optimizelyDomainTestCookie"]};
...
SfdcWwwBase.gdpr.hasAdvertising.resolve()
}}}})
}return f
};
...
if(SfdcWwwBase.gdpr.isAdvertisingActive()){window.optimizely.push({type:"optOut",isOptOut:false});
Util.deleteCookie("optimizelyOptOut")
}else{window.optimizely.push({type:"optOut",isOptOut:true});
Util.deleteCookie("optimizelyEndUserId")
}};
...
},createUUID:function(){if(("crypto" in window)&&("randomUUID" in crypto)){return crypto.randomUUID()
...
}var e="api.company-target.com/api/v2/ip.json?token=ISTSgTgnTaAqQ1ubW6LBngVUhd6V0uEuIo36F2qy",a=Util.getParam("ipOverride"),d=document.location.protocol,b=d+"//"+e+(a!==""?"&query="+a:"");
