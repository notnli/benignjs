...
(() => {
  ...
  class CountrySelector {
    constructor(element) {
      ...
      this.settings = {
        SITECD: document.querySelector('#siteCode') ? document.querySelector('#siteCode').value : 'test',
        selector: {
          content: '.nv16-country-selector__content-wrap',
          countryList: '.nv16-country-selector__menu .menu__select',
          urlSitecd: '[data-country-type=sitecd]',
          locationSitecd: '[data-country-type=location]',
          continue: '[data-action=countrySelectorContinue]',
          close: '[data-action=countrySelectorClose]',
        },
        cookies: {
          accept: {
      ...
      CountrySelector.instance.set(element, this);
    ...
    init() {
      ...
      if( this.isAccepted() || this.isLocalSite() || this.isCountrySelected() || !this.checkCountryValid() ) {
        this.remove();
      } else {
        this.show();
      }
    ...
    setElements() {
      ...
      // Option 중복 제거
      this.els.countryList.find(this.settings.selector.locationSitecd).target.reduce((acc, curr) => {
        const optionValue = curr.value;
        if (acc[optionValue]) {
          curr.remove();
        } else {
          acc[optionValue] = true;
        }
        return acc;
      }, {});
    ...
    show() {
      this.els.section.css({ height: `${this.els.content.height()}px` });
      this.els.section.slideDown(400, () => {
        this.els.section.addClass('nv16-country-selector--init');
        this.els.section.css({ height: 'auto' });
      });
    }
    ...
    close() {
      ...
      //slideUp and remove
      this.els.section.css({ height: `${this.els.section.height()}px` });
      this.els.section.slideUp(350, this.remove.bind(this));
    }
  
    handleContinue() {
      const selectedValue = this.els.countryList.target[0].querySelector('option[selected]').getAttribute('value');

      if( selectedValue === 'other' ) {
        window.location.href = `${window.location.origin}/${this.settings.SITECD}${this.settings.otherCountry}`;
      } else {
        utils.setCookie(this.settings.cookies.selectedSitecd.name, selectedValue, this.settings.cookies.selectedSitecd.days);
        this.setAcceptCookie();
        this.close();
      }
    }
    ...
    setAcceptCookie() {
      const cookieAccept = this.settings.cookies.accept;
      utils.setCookie(cookieAccept.name, cookieAccept.value, cookieAccept.days);
    }
    ...
    isAccepted() {
      return utils.getCookie(this.settings.cookies.accept.name) ? true : false;
    }

    isCountrySelected() {
      return this.settings.SITECD === utils.getCookie(this.settings.cookies.selectedSitecd.name) ? true : false;
    }

    // location 연결 istecd와 url sitecd가 일치 (해당 국가가 이미 셀렉터 option으로 들어 온 것으로 간주)
    isLocalSite() {
      const location = this.els.countryList.find(this.settings.selector.locationSitecd).target.map((_el) => _el.value);
      return !!location.find((_) => _ === this.settings.SITECD);
    }

    // url sitecd 정보와 location에 일치하는 sitecd 둘 중 하나라도 없으면 false 반환, 둘다 있어야 true
    checkCountryValid() {
      return this.els.countryList.find(this.settings.selector.urlSitecd).target.length && this.els.countryList.find(this.settings.selector.locationSitecd).target.length ? true : false;
    }
  }

  CountrySelector.instance = new WeakMap();


  function init() {
    $q(selector.section).target.forEach((element) => {
      if(!CountrySelector.instance.has(element)) {
  ...
  function reInit() {
    $q(selector.section).target.forEach((element) => {
      if(CountrySelector.instance.has(element)) {
        CountrySelector.instance.get(element).reInit();
      }
    });
  }
  ...
  function show() {
    $q(selector.section).target.forEach((element) => {
      if(CountrySelector.instance.has(element)) {
        CountrySelector.instance.get(element).show();
      }
    });
  }
  ...
  function close() {
    $q(selector.section).target.forEach((element) => {
      if(CountrySelector.instance.has(element)) {
        CountrySelector.instance.get(element).close();
      }
    });
  }
